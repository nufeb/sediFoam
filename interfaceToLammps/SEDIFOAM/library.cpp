...
int lammps_get_global_n(void *handle)
{
  return lammps_get_natoms(handle);
}
...
void lammps_get_initial_np(void* handle, int* np_)
{
  ...
  MPI_Allreduce(copyNp, np_, mysize, MPI_INT, MPI_SUM, lmp->world);
...
// Not extened to provide particle diameter, density, and type as well.
// The name will, however, not be updated.
void lammps_get_initial_info(void* handle, double* coords, double* velos, double* diam,
...
int lammps_get_local_n(void* handle)
{
  auto *lmp = (LAMMPS *) handle;
...
void lammps_get_local_domain(void* handle, double* domain_)
{
  int myrank;
  MPI_Comm_rank(MPI_COMM_WORLD, &myrank);
  ...
  double *sublo = lmp->domain->sublo;
  double *subhi = lmp->domain->subhi;
...
void lammps_get_local_info(void* handle, double* coords_, double* velos_,
                           int* foamCpuId_, int* lmpCpuId_, int* tag_)
{
  ...
  int i;
  for (i = 0; i < (lmp->modify->nfix); i++)
    if (strcmp(lmp->modify->fix[i]->style,"fdrag") == 0) break;

  if (i < lmp->modify->nfix)
    //initialize the pointer
    drag_ptr = (FixFluidDrag *) lmp->modify->fix[i];
...
void lammps_put_local_info(void* handle, int nLocalIn, double* fdrag,
...
void lammps_step(void *handle, int n)
{
  auto *lmp = (LAMMPS *) handle;
  // lmp -> Run -> command(5, n, "pre", "no", "post", "no");
  char cmd[50];
  int nc;
...
double lammps_get_timestep(void *handle)
{
  auto *lmp = (LAMMPS *) handle;
  return lmp->update->dt;
}

/* ---------------------------------------------------------------------- */

void lammps_set_timestep(void *handle, double dt_i)
{
  auto *lmp = (LAMMPS *) handle;
  lmp->update->dt = dt_i;
}
...
void lammps_create_particle(void* handle, int npAdd, double* position, double* tag,
                            double diameter, double rho, int type, double* vel)
{
  ...
  int natom = static_cast<int> (lmp->atom->natoms);
  double **x = lmp->atom->x;
  int ntype = type;
  ...
  tagint *tagOld = lmp->atom->tag;
  for (int i = 0; i < lmp->atom->nlocal; i++) max = MAX(max,tagOld[i]);
  // MPI_Allreduce(&max,&maxtag_all,1,MPI_LMP_TAGINT,MPI_MAX,lmp->world);
  ...
  for (int m = 0; m < npAdd; m++) {
    ...
    lmp->atom->avec->create_atom(ntype,xNew);
    int nlocal = lmp->atom->nlocal;

    int n = lmp->atom->nlocal - 1;
    // lmp->atom->tag[n] = maxtag_all + 1;
    lmp->atom->tag[n] = tag[m];
    ...
    int igroup = lmp->group->find("active");
    int groupbit = lmp->group->bitmask[igroup];

    lmp->atom->mask[n] = 1 | groupbit;
    lmp->atom->image[n] = ((imageint) IMGMAX << IMG2BITS) |
        ((imageint) IMGMAX << IMGBITS) | IMGMAX;
    lmp->atom->v[n][0] = vel[0];
    lmp->atom->v[n][1] = vel[1];
    lmp->atom->v[n][2] = vel[2];

    double radtmp = 0.5*diameter;
    lmp->atom->radius[n] = radtmp;
    lmp->atom->rmass[n] = 4.0*3.14159265358917323846/3.0 * radtmp*radtmp*radtmp * rho;

    for (int j = 0; j < lmp->modify->nfix; j++)
    {
      if (lmp->modify->fix[j]->create_attribute)
      {
        lmp->modify->fix[j]->set_arrays(n);
      }
    }
  }
  ...
  int nGlobal = 0;
  int nlocal = lmp->atom->nlocal;
  MPI_Allreduce(&nlocal,&nGlobal,1,MPI_INT,MPI_SUM,lmp->world);
  lmp->atom->natoms = nGlobal;
  ...
  for (int j = 0; j < lmp->modify->nfix; j++)
  {
    bigint nt = lmp->update->ntimestep;
    lmp->modify->fix[j]->next_reneighbor = nt + 1;
  }
...
void lammps_delete_particle(void *handle, int* deleteList, int nDelete)
{
  int myrank;
  MPI_Comm_rank(MPI_COMM_WORLD, &myrank);

  auto *lmp = (LAMMPS *) handle;
  class RanPark *random = new RanPark(lmp,100);
  ...
  int nmax = 0;
  if (lmp->atom->nlocal > nmax) {
    lmp->memory->destroy(list);
    lmp->memory->destroy(mark);
    nmax = lmp->atom->nmax;
    lmp->memory->create(list,nmax,"evaporate:list");
    lmp->memory->create(mark,nmax,"evaporate:mark");
    del_flag = 1;
  }
  ...
  int nall,nbefore;
  MPI_Allreduce(&ncount,&nall,1,MPI_INT,MPI_SUM,lmp->world);

  MPI_Scan(&ncount,&nbefore,1,MPI_INT,MPI_SUM,lmp->world);
  nbefore -= ncount;
  ...
  double totalFlux = 0;
  MPI_Allreduce(&flux,&totalFlux,1,MPI_DOUBLE,MPI_SUM,lmp->world);
  ...
  AtomVec *avec = lmp->atom->avec;

  for (i = nlocal-1; i >= 0; i--) {
    if (mark[i]) {
      avec->copy(lmp->atom->nlocal-1,i,1);
      lmp->atom->nlocal--;
    }
  }
  ...
  lmp->atom->natoms -= ndel;

  if (ndel && lmp->atom->map_style) {
    lmp->atom->nghost = 0;
    lmp->atom->map_init();
    lmp->atom->map_set();
  }
  ...
  for (int j = 0; j < lmp->modify->nfix; j++)
  {
    bigint nt = lmp->update->ntimestep;
    lmp->modify->fix[j]->next_reneighbor = nt + 1;
    // lmp->modify->fix[j]->force_reneighbor = 1;
  }
