...
FixWallGranFix::FixWallGranFix(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  if (narg < 10) error->all(FLERR,"Illegal fix wall/granFix command");

  if (!atom->sphere_flag)
    error->all(FLERR,"Fix wall/granFix requires atom style sphere");
  ...
  if (kn < 0.0 || kt < 0.0 || gamman < 0.0 || gammat < 0.0 ||
    ...
    error->all(FLERR,"Illegal fix wall/granFix command");

  // convert Kn and Kt from pressure units to force/distance^2 if Hertzian

  if (force->pair_match("gran/hertzFix/history",1)) {
    kn /= force->nktv2p;
    kt /= force->nktv2p;
  }
  ...
  if (strcmp(arg[iarg],"xplane") == 0) {
    if (narg < iarg+3) error->all(FLERR,"Illegal fix wall/granFix command");
    wallstyle = XPLANE;
    ...
    if (narg < iarg+2) error->all(FLERR,"Illegal fix wall/granFix command");
    wallstyle = ZCYLINDER;
    lo = hi = 0.0;
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"wiggle") == 0) {
      if (iarg+4 > narg) error->all(FLERR,"Illegal fix wall/granFix command");
      if (strcmp(arg[iarg+1],"x") == 0) axis = 0;
      else if (strcmp(arg[iarg+1],"y") == 0) axis = 1;
      else if (strcmp(arg[iarg+1],"z") == 0) axis = 2;
      else error->all(FLERR,"Illegal fix wall/granFix command");
      ...
      if (iarg+3 > narg) error->all(FLERR,"Illegal fix wall/granFix command");
      if (strcmp(arg[iarg+1],"x") == 0) axis = 0;
      else if (strcmp(arg[iarg+1],"y") == 0) axis = 1;
      else if (strcmp(arg[iarg+1],"z") == 0) axis = 2;
      else error->all(FLERR,"Illegal fix wall/granFix command");
  ...
  if (wiggle && wshear)
    error->all(FLERR,"Cannot wiggle and shear fix wall/granFix");
  if (wiggle && wallstyle == ZCYLINDER && axis != 2)
    error->all(FLERR,"Invalid wiggle direction for fix wall/granFix");
  if (wshear && wallstyle == XPLANE && axis == 0)
    error->all(FLERR,"Invalid shear direction for fix wall/granFix");
...
void FixWallGranFix::init()
{
  ...
  if (force->pair_match("gran/hooke",1))
    ...
    pairstyle = HOOKE_HISTORY;
  else if (force->pair_match("gran/hertzFix/history",1))
    pairstyle = HERTZ_HISTORY;
  else if (force->pair_match("gran/hertzFix/history/omp",1))
    pairstyle = HERTZ_HISTORY;
  else error->all(FLERR,"Fix wall/granFix is incompatible with Pair style");
}

/* ---------------------------------------------------------------------- */

void FixWallGranFix::setup(int vflag)
{
...
void FixWallGranFix::post_force(int vflag)
{
...
void FixWallGranFix::post_force_respa(int vflag, int ilevel, int iloop)
{
...
void FixWallGranFix::hooke(double rsq, double dx, double dy, double dz,
...
void FixWallGranFix::hooke_history(double rsq, double dx, double dy, double dz,
                                double *vwall, double *v,
                                double *f, double *omega, double *torque,
...
void FixWallGranFix::hertz_history(double rsq, double dx, double dy, double dz,
                                double *vwall, double *v,
                                double *f, double *omega, double *torque,
                                double radius, double mass, double *shear)
{
  ...
  sn = 2.0*1.0/1.82*kn*sqrt((radius-r)*radius);
  st = 8.0*1.0/8.84*kn*sqrt((radius-r)*radius);
  ...
  ccel = polyhertz*4.0/5.46*kn*(radius-r)*rinv - sqrt(sn*meff)*damp;
...
double FixWallGranFix::memory_usage()
{
  int nmax = atom->nmax;
...
void FixWallGranFix::grow_arrays(int nmax)
{
  memory->grow(shear,nmax,3,"fix_wall_granFix:shear");
}
...
void FixWallGranFix::copy_arrays(int i, int j, int delflag)
{
  shear[j][0] = shear[i][0];
  shear[j][1] = shear[i][1];
  shear[j][2] = shear[i][2];
...
void FixWallGranFix::set_arrays(int i)
{
  shear[i][0] = shear[i][1] = shear[i][2] = 0.0;
}
...
int FixWallGranFix::pack_exchange(int i, double *buf)
{
  buf[0] = shear[i][0];
  buf[1] = shear[i][1];
  buf[2] = shear[i][2];
...
int FixWallGranFix::unpack_exchange(int nlocal, double *buf)
{
...
int FixWallGranFix::pack_restart(int i, double *buf)
{
  int m = 0;
  buf[m++] = 4;
  buf[m++] = shear[i][0];
...
void FixWallGranFix::unpack_restart(int nlocal, int nth)
{
...
int FixWallGranFix::maxsize_restart()
{
  return 4;
}
...
int FixWallGranFix::size_restart(int nlocal)
{
  return 4;
}
